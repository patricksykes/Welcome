...
## Variable Choice
This has taken me a while, but I think it’s important. I have read through the attached table – each tab has different model variables on different time scales. The tabs are in alphabetical order. The key ones are:

The ones starting with “O” are for Ocean, and then followed by the timescale (clim = climatology, day, dec = decade, mon = month, yr). I think we should go with monthly (this is what Isaac has used) and also get what we can from the daily data (for marine heatwaves for example). The Omon runs have 292 variables and the Oday runs have only 8 variables
...
**Priority 1** (Temperature, pH and O2 at surface and throughout the water column) – particularly for Isaac for Aquamaps models
Omon
